...
from .forms import EncomiendaForm,LiquidacionForm,liquidacionRecepcionForm,ClienteRecepcionForm
...
def encomiendaAdd(request):    
    numEncomienda = AgenciaDocumento.objects.get(agencia=request.user.agencia.get(id=request.session['agencia_id']),
    ...
    if request.method == 'POST':
        ...
        if detalle.is_valid():
            ...
            detencomienda.subTotal = detencomienda.cantidad * detencomienda.valorUnitario
            detencomienda.save()

            correlativoAgeUpd = AgenciaDocumento.objects.get(agencia=request.user.agencia.get(id=request.session['agencia_id']),
                                                       documento__codigo='EN')
            ...
            encomienda = dict(
                        ...
                        esContraEntrega = True if request.POST.get('esContraEntrega',False) == 'on' else False,
                        aDomicilio = True if request.POST.get('aDomicilio',False)=='on' else False,
                        domicilio = request.POST['domicilio'],
                        seguridadClave = request.POST['seguridadClave'],
                        observacion = request.POST['observacion'],
                        estado = 'agenciaOrigen',
                        precio = detalle.cleaned_data['cantidad'] * detalle.cleaned_data['valorUnitario'],
            ...
            if encomienda.esContraEntrega:                
                ...
                parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'encomiendaSalida','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','encom_pk':idencom,'destCountry':'PER&opId=0&busType=Any'}
                return redirect(f"{reverse('envio:encomienda-print')}?{urlencode(parametro)}")

            else:
                idmov =venta.id
                parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'encomiendaSalida','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','mov':idmov,'destCountry':'PER&opId=0&busType=Any'}
                return redirect(f"{reverse('tesoreria:tesoreria-movimiento-add')}?{urlencode(parametro)}")        
        ...
        if form_clie_recp.is_valid():            
            if encomienda.seguridadClave != form_clie_recp.cleaned_data.get('clave'):
                ...
                if encomienda.esContraEntrega:
                    ...
                    parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'encomiendaLlegada','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','mov':idmov,'destCountry':'PER&opId=0&busType=Any'}
                    return redirect(f"{reverse('tesoreria:movimiento-add')}?{urlencode(parametro)}")

                
                
    context = {
        'encomienda':encomienda,
        'form_clie_recp':form_clie_recp
    }
...
def encomiendaPrint453(request):
    ...
    high = 520
    for item in encomienda.venta.detallemov_set.all():
        enco = [Paragraph(item.descripcion,styleN),item.cantidad,item.valorUnitario,item.subTotal]
        data.append(enco)
        high=high-18
...
def encabezado(pdf,encomienda):
    ...
    if encomienda.esContraEntrega: pdf.drawString(300, 577, f': Contra Entrega')
    else: pdf.drawString(300, 577, f': Contado')
...
def encomiendaPrint(request):
    encom_pk = request.GET.get('encom_pk')
    encomienda = get_object_or_404(Encomienda, pk=encom_pk)
    total = encomienda.venta.detallemov_set.all().aggregate(Sum('subTotal'))['subTotal__sum']
...
def liquidacionAdd(request):
    ...
    if request.method == 'POST':
        ...
        if liquidacion.is_valid():
            ...
            id_liq =liq.id
            parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'encomiendaSalida','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','li_pk':id_liq,'destCountry':'PER&opId=0&busType=Any'}
            return redirect(f"{reverse('envio:liquidacion-add-encomienda')}?{urlencode(parametro)}")


    context={
        'liquidacion' : liquidacion,
        'correlativo': correlativoAgeUpd.correlativo
    }
    ...
    if data.finalizado:
        ...
        if request.method == 'POST':
            ...
            if form_liquidacion.is_valid():
                ...
                p.save(update_fields=['agenciaOrigen','agenciaDestino','vehiculo','conductor'])
...
def liquidacionAddEncomienda(request):
    ...
    encomienda = Encomienda.objects.filter(agenciaOrigen=liquidacion.agenciaOrigen,  
                                           estado='agenciaOrigen').exclude(
                                               pk__in = liquidacion.encomienda.all().values('id'))
    
    if request.method == 'POST':
        encom=request.POST.getlist('_selected_action')
        liquidacion.encomienda.add(*encom)
        parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomovi':'tipomovi','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','li_pk':liquidacion_pk,'destCountry':'PER&opId=0&busType=Any'}
...
def liquidacionRecepcion(request):    
    ...
    try:
        ...
        form_liq_recep = liquidacionRecepcionForm(instance=liquidacion.liquidacionrecepcion)
...
def liquidacionPrint(request):
    liquidacion_pk = request.GET.get('li_pk')
    liquidacion = get_object_or_404(Liquidacion,pk=liquidacion_pk)
    encomiendas = liquidacion.encomienda.all().order_by('-agenciaDestino','esContraEntrega')
    suma_directa = liquidacion.encomienda.filter(esContraEntrega=False).aggregate(Sum('precio'))['precio__sum']
    suma_contra_entrega = liquidacion.encomienda.filter(esContraEntrega=True).aggregate(Sum('precio'))['precio__sum']
    if suma_directa is None: suma_directa=0 
    if suma_contra_entrega is None: suma_contra_entrega=0
