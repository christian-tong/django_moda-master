...
class Encomienda(models.Model):
    ...
    remite = models.ForeignKey(Persona, on_delete=models.PROTECT,  related_name='remite_persona')
    consignado = models.ForeignKey(Persona, on_delete=models.PROTECT, related_name='consignado_persona')
    agenciaOrigen = models.ForeignKey(Agencia, on_delete=models.PROTECT)
    agenciaDestino = models.ForeignKey(Agencia, on_delete=models.PROTECT, related_name='destino_agencia')
    esContraEntrega = models.BooleanField(verbose_name='¿Contra entrega?',default=False) #es = true no es=false
    aDomicilio = models.BooleanField(verbose_name='¿A domicilio?', default=False)
    domicilio = models.CharField(max_length=200,verbose_name='Direccion del domicilio', blank=True,null=True)
    ...
    precio = models.DecimalField(max_digits=7,decimal_places=2,default=0,blank=True,null=True)
    numeroContacto = models.CharField(verbose_name="Telefono", max_length=50, blank=True,null=True)
    class Meta:
...
class ClienteRecepcion(models.Model):
    encomienda = models.OneToOneField(Encomienda, on_delete=models.PROTECT,blank=True,null=True)
...
class Liquidacion(models.Model):
    ...
    usuario = models.ForeignKey(Persona, on_delete=models.PROTECT,blank=True,null=True)
    agenciaOrigen =  models.ForeignKey(Agencia, on_delete=models.PROTECT)
    agenciaDestino = models.ForeignKey(Agencia, on_delete=models.PROTECT, related_name='agencia_destino')
...
class liquidacionRecepcion(models.Model):
    usuario = models.ForeignKey(Persona, on_delete=models.PROTECT,blank=True,null=True)
    liquidacion = models.OneToOneField(Liquidacion, on_delete=models.PROTECT,blank=True,null=True)
    fecha = models.DateTimeField(auto_now=True)
