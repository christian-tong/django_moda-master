...
def movimiento_add(request):
    ...
    if tipomovi == "ventaPasaje":
        ...
        detalle = (
            f"Venta de pasaje, ruta: {venta.embarque.programacionViaje.nombreViaje}"
        )
    ...
    MedioPagoFormSet = formset_factory(
        MedioPagoForm, formset=BaseMedioPagoFormSet, extra=0
    )
    formset = MedioPagoFormSet(
        initial=[
            {"tipoMedioPago": 1, "monto": montoCobrar},
        ]
    )
            ...
            with transaction.atomic():
                ...
                if factura.is_valid() and formset.is_valid():
                    caja = Caja.objects.get(
                        agencia=request.user.agencia.get(
                            id=request.session["agencia_id"]
                        )
                    )
                    ...
                    return redirect(
                        f"{reverse('facturacion:boleta-print')}?{urlencode(parametro)}"
                    )
        except Exception as e:
            print(e)
            messages.error(
                request, f"Algo salio mal al momento de guardar los datos(Error: {e}"
            )

    context = {"factura": factura, "medioPagos": formset, "montoCobrar": montoCobrar}
