...
class CajaForm(forms.ModelForm):
    cajeros = forms.ModelMultipleChoiceField(queryset = Persona.objects.filter(ispersonal=True),required=True) 
...
class MovimientoCajaForm(forms.ModelForm):
    cliente = forms.ModelChoiceField(queryset=Persona.objects.filter(numDoc='44319644'), empty_label="Select an author")
    class Meta:
        model = MovimientoCaja
        fields = '__all__'
        
        widgets ={
            'movFecha': forms.DateInput(attrs={'type': 'date'})
        }
