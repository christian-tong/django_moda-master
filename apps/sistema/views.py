        ...
        if dataLogin.is_valid():    
            ...
            if user is not None and user.is_active:
                ...
                if ag:
                    ...
                    messages.error(request,'Accedio con exito, pero no tiene una agencia a cargo!!')    
            else:
...
def menuList(request):
    ...
    if request.method == "GET" and request.is_ajax():
        ...
        if user.is_superuser:
            ...
            user_permissions = user.user_permissions.all()  # Permisos individuales del usuario
            group_permissions = Permission.objects.filter(group__user=user)  # Permisos de los grupos del usuario
            all_permissions = user_permissions | group_permissions  # Combinar ambos

            # Filtrar menús que coincidan con los permisos del usuario
            menus = Menu.objects.filter(padre=None, permiso__in=all_permissions).order_by('orden')"""


        menus = Menu.objects.filter(padre=None).order_by('orden') #por boorar al activar el comentado
        items={}
        
        for menu in menus: 
            subitem={}           
            subitem['padre']=list(Menu.objects.filter(id=menu.id).values())
            subitem['hijo']  = list(Menu.objects.filter(padre=menu).order_by('orden').values()) #por boorar al activar el comentado
            """if user.is_superuser:
                subitem['hijo']  = list(Menu.objects.filter(padre=menu).order_by('orden').values()) 
            else:
                subitem['hijo']  = list(Menu.objects.filter(padre=menu, permiso__in=all_permissions).order_by('orden').values())
"""
    ...
    #return HttpResponse(data) los dos funcionan par respuesta en ajax
...
def homeIndex(request):
    ...
    stingresos = MovimientoCaja.objects.filter(tipoMov__tipo='ingreso').aggregate(Sum('monto'))['monto__sum']
    tingresos = stingresos if stingresos !=None else 0
    stegresos = MovimientoCaja.objects.filter(tipoMov__tipo='egreso').aggregate(Sum('monto'))['monto__sum']
