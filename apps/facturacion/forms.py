...
class FacturaBoletaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['tipoDocumento'].queryset = TipoDocumento.objects.filter(activo=True)
        # Si se proporciona un cliente, configurar el queryset para solo ese cliente
        if cliente_inicial:
            self.fields['cliente'].queryset = Persona.objects.filter(pk=cliente_inicial.id)
            self.initial['cliente'] = cliente_inicial  # Establecer cliente inicial en el formulario
