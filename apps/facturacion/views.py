...
def list(request):
    ...
    if q:
        print(q)
        print(type(q))
        data = FaturaBoleta.objects.filter(
            Q(cliente__denominacion__icontains=q)|
            Q(cliente__numDoc__icontains=q)
            ).order_by('-fechaFact').distinct()
        add_ruta_get=f'&q={q}'
...
def boletaPrint(request):
    ...
    if not factura.estaFacturado:
        ...
        if req.get('errors'):
            ...
            messages.error(request,f"No se ah podido facturar, le enviaremos mas tarde a su correo o a su whatsapp <br> {req['errors']}")
            image_qr=None
        else:
            messages.success(request,f"La facturación fue exitosa")
...
def ComprobanteExcel(request):
    ...
    columns = ['Tipo Comprobante','Serie','Numero','Cliente','Tipo Documento','Numero','Monto','Fecha','Detalle']
    obj = FaturaBoleta.objects.filter(fechaFact__year=year, fechaFact__month=mes,estaFacturado=True)\
        .values_list(
        'tipoDocumento__descripcion','serie','numero','cliente__denominacion',
        'cliente__tipoDoc__descripcion','cliente__numDoc','monto','fechaFact','ventaMovimiento__detallemov__descripcion')
