...
from .models import Embarque, Manifiesto, ProgramacionAsiento, ProgramacionViaje
from .forms import EmbarqueForm,ProgramacionViajeForm,BuscarViajeForm,ManifiestoForm,EmbarqueEditForm,ProgramacionAsientoForm
...
def embarqueAdd(request):
    ...
    if idPrograma:
        ...
        asientoProgramado = ProgramacionAsiento.objects.filter(programacionViaje=viajeProgramado).order_by('asiento__codigoMatrix')
        embarque = EmbarqueForm(initial=
        ...
        if request.method == 'POST':
            ...
            idmov =venta.id
            parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'ventaPasaje','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','mov':idmov,'destCountry':'PER&opId=0&busType=Any'}
...
def embarqueCambiar(request):
    ...
    if id:
        ...
        if embarque:
            ...
            rutas = ProgramacionViaje.objects.filter(rutaOrigen = embarque.programacionViaje.rutaOrigen,
    ...
    if request.is_ajax():
        ...
        if idprograma:
            data=[]
            asientos = ProgramacionAsiento.objects.filter(programacionViaje__pk=idprograma).order_by('asiento__codigoMatrix')
            nas = ''
            try:
                nas =  math.floor(11/asientos.first().programacionViaje.vehiculo.numColumnas)
            except:                
                messages.info(request,"No tiene Asientos programados")
            context={
                'asientoProgramado':asientos,
                'anchoColumna':nas
            }
    ...
    if request.method =='POST':
        ...
        if embarque_form.is_valid():
            ...
            asiento_program = get_object_or_404(ProgramacionAsiento,pk=request.POST.get('idasiento'))
...
def programa_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            idprogr =prog.id
            parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'encomiendaSalida','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','idproruta':idprogr,'destCountry':'PER&opId=0&busType=Any'}
            return redirect(f"{reverse('viaje:programa-asiento-list')}?{urlencode(parametro)}")
...
def programaEdit(request):
    ...
    programa = ProgramacionViajeForm(instance = qr,initial={'fechaViaje':fecha,'horaViaje':hora})
...
def programaAsientoEdit(request):
    ...
    if request.method == 'POST':
        ...
        if form_prog_asiento.is_valid():
            ...
            parametro={'fromCityName':'chintucaycuri%20%28Todos%29&fromCityId=195648','tipomov':'ventaPasaje','toCityName':'Lima%20%28Todos%29&toCityId=195105&onward=20-Jan-2022&srcCountry=PER','idproruta':id,'destCountry':'PER&opId=0&busType=Any'}
            return redirect(f"{reverse('viaje:programa-asiento-list')}?{urlencode(parametro)}")

    context = {
        'form_prog_asiento':form_prog_asiento,
        'asiento': asiento
...
def manifiestoEdit(request):
    ...
    form = ManifiestoForm(instance=manifiesto,initial={'fechaViaje':manifiesto.programacionViaje.fechaViaje.isoformat()})
