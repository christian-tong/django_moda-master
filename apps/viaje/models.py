...
class ProgramacionViaje(models.Model):
    ...
    rutaDestino = models.ForeignKey(
        Agencia, on_delete=models.PROTECT, related_name="destino_viaje"
    )
    fechaViaje = models.DateField()
    horaViaje = models.TimeField(default="12:00")
    vehiculo = models.ForeignKey(Vehiculo, on_delete=models.PROTECT)
    piloto = models.ForeignKey(
        Conductor, on_delete=models.PROTECT, blank=True, null=True
    )
...
class ProgramacionAsiento(models.Model):
    programacionViaje = models.ForeignKey(
        ProgramacionViaje, on_delete=models.PROTECT, blank=True, null=True
    )
    asiento = models.ForeignKey(
        Asiento, on_delete=models.PROTECT, blank=True, null=True
    )
...
class Embarque(models.Model):
    ...
    lugar_abordo = models.ForeignKey(
        Agencia,
    ...
    lugar_bajada = models.ForeignKey(
        Agencia,
    ...
    enSala = models.BooleanField(
        verbose_name="El pasajero esta en sala?", default=False
    )
...
class Manifiesto(models.Model):
    ...
    vehiculo = models.ForeignKey(
        Vehiculo, on_delete=models.PROTECT, blank=True, null=True
    )
    piloto = models.ForeignKey(
        Conductor, on_delete=models.PROTECT, blank=True, null=True
    )
    ...
    programacionViaje = models.OneToOneField(
        ProgramacionViaje, on_delete=models.PROTECT, blank=True, null=True
    )
    usuario = models.ForeignKey(
        Persona, on_delete=models.PROTECT, blank=True, null=True
    )
    modalidaServicio = models.CharField(
        max_length=30, default="Regular", blank=True, null=True
    )
    seGenero = models.BooleanField(default=False)
    embarque = models.ManyToManyField(Embarque, blank=True)
    printRutaOrigen = models.ForeignKey(
        Agencia,
    ...
    printRutaFinal = models.ManyToManyField(
        Agencia,
        through="ManifiestoRutaFinal",
