...
def logicaEnvioOse(pk,factura,canje):
    ...
    response = requests.post(url,data = json.dumps(data['documento'],cls=CustomJsonEncoder), headers=head)
    ...
    if req.get('errors') is None:   
        ...
        correlativo = AgenciaDocumento.objects.get(documento=movimiento.faturaboleta.tipoDocumento.pk,
...
def archivoJsonOse(pk):
    movimiento = get_object_or_404(Movimiento,pk=pk)
    data={'error':True}
    if movimiento.faturaboleta.tipoDocumento.codigo =='01' or movimiento.faturaboleta.tipoDocumento.codigo =='03':
        
        correlat = AgenciaDocumento.objects.get(documento=movimiento.faturaboleta.tipoDocumento.pk,
                                            agencia__tipo="PRINCIPAL")
        ...
        documento = {
            ...
            "cliente_direccion":"-" if movimiento.faturaboleta.cliente.direccion is None else f'{movimiento.faturaboleta.cliente.direccion}',
            "cliente_email": "",
            "cliente_email_1": "",
            "cliente_email_2": "",
